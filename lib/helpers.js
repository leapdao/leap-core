...
import { BigInt, add, subtract } from 'jsbi';
...
export function calcInputs(unspent, from, amount, color) {
  ...
  for (let i = 0; i < myUnspent.length; i += 1) {
    ...
    sum = add(sum, myUnspent[i].output.value);
...
export function calcOutputs(unspent, inputs, from, to, amount, color) {
  ...
  const sum = unspent.filter(inInputs).reduce((a, u) => add(a, u.output.value), BigInt(0));
  ...
  if (sum > amount) {
    outputs.push(new Output(subtract(sum, amount), from.toLowerCase(), color));
  }

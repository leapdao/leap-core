...
function isTransfeEvent(log){
  return Array.isArray(log) && (log.length === 3) &&
              Buffer.isBuffer(log[0]) && (log[0].length === 20) &&
              Array.isArray(log[1]) && (log[1].length === 3) &&
                 Buffer.isBuffer(log[1][0]) && (log[1][0].length === 32) &&
                 Buffer.isBuffer(log[1][1]) && (log[1][1].length === 32) &&
                 Buffer.isBuffer(log[1][2]) && (log[1][2].length === 32) &&
              Buffer.isBuffer(log[2]) &&(log[2].length === 32)
}

// user should provide erc20 addresses used in tx's spend condition contracts,  
module.exports = async (tx, erc20sToBeUsed, unspent) => {
  ...
  for (let i = 0; i < tx.inputs.length; i += 1) {
    inputMap[tx.inputs[i].prevout.getUtxoId()] =
      unspent[tx.inputs[i].prevout.hex()];
  }
  ...
  // eslint-disable-next-line guard-for-in
  for (const key in unspent) {
    // minting amount of output to address of condition
    const unspentItem = unspent[key]
  ...
  for (let i = 0; i < tx.inputs.length; i += 1) {
    ...
    results.vm.logs.filter(isTransfeEvent).forEach(log => {
  ...
  Object.keys(unspent).forEach(item => {
    transformedUnspent.push({output: Output.fromJSON(unspent[item]), outpoint: Outpoint.fromRaw(item)})
  })

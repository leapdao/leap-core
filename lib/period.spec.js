...
describe('periods', () => {
  ...
  it('should allow to get proof from period with prevPeriod.', (done) => {
    ...
    const [currentPeriod, prevPeriod] = Period.evaluatePrevPeriodProof(proof);
    expect(prevPeriod).to.eql(periodAfter.periodRoot());
    expect(currentPeriod).to.eql(period.periodRoot());
    done();
  });

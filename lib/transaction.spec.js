...
describe('transactions', () => {
  ...
  it('should allow to create and parse transfer tx.', () => {
    ...
    // test signing
    const transferHash = '0x9c09769c6741d5f563d835a2f7e3629b67d2a20a45e132e88ffe462c0196afeb';
    transfer.sign([PRIV]);
  ...
  it('should allow to serialze and deserialize transfer tx to/from json.', () => {
    ...
    expect(json).to.eql({
      ...
      options: {
        signatures: [
          '0x1c194a1793ccab3c2377def10aadb7bc6c4565203265b0a460938d1812a6df59c5081200dcd2bff81f940edd9974291e641e0c369eca2c6990b6dec8857659d774',
        ]
      },
      outputs: [{ address: ADDR, value: value.toString(), color }],
    });
  ...
  it('should allow to create and parse transfer tx with 2 inputs.', () => {
    ...
    // test signing
    const transferHash = '0xda057ef882cdb7374943085804aab2d1695b05dcf300190378eb57d178e23c67';
  ...
  it('should allow to create and parse transfer tx with 2 outputs.', () => {
    ...
    // test signing
    const transferHash = '0x13e54fe36a24b535f3ade36de492f233b6c0b9ae578f4cd9063d815a3f417ff5';
    transfer.sign([PRIV]);
    expect(transfer.hash()).to.eql(transferHash);
    // test parse
    assert.deepEqual(Tx.fromRaw(transfer.toRaw()), transfer);
  ...
  it('should allow to create and parse spending condition tx.', () => {
    ...
    const conditionHash = '0x0ee75c1777bd6a784615b7e5076d00cf35e196b417fb3bd4d84db2ec1266da61';
  ...
  it('should allow to create and parse spending condition with different tokens', () => {
    ...
    // test hashing and parsing
    const conditionHash = '0x01843a5607d8583fbb0c3ab4324b971e47f0a9ea43ec87c5fb788d4e261d7100';
    expect(condition.hash()).to.eql(conditionHash);
  ...
  it('should allow to sign with web3', async () => {
    ...
    assert.deepEqual(tx.sigData(), '0x41454c232c8528f232eddf311c208e3169df7ea1bc05118ec965632c9c644246');
    ...
    const sig = Buffer.from('d0c96bd05190ff0fd7d7ea2359ceb4135481a5a1f147966f0dec001c80785bb46e1b20abf721d23559b906436dbcbe3a091d7a36bc37000e54e9748422e5bde11b', 'hex');
    const v = sig[64];
    const r = sig.slice(0, 32);
    const s = sig.slice(32, 64);

    assert.deepEqual(tx.options.signatures[0], { v, r, s });
  });

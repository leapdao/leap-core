...
describe('transactions', () => {
  ...
  it('should allow to create and parse validatorJoin tx.', () => {
    ...
    const validatorJoin = Tx.validatorJoin(slotId, tenderKey, 10);
  ...
  it('should allow to create and parse validatorLogout tx.', () => {
    const slotId = 0;
    const tenderKey = '0x7640D69D9EDB21592CBDF4CC49956EA53E59656FC2D8BBD1AE3F427BF67D47FA';
    const validatorLogout = Tx.validatorLogout(slotId, tenderKey, 10, 5);
    assert.deepEqual(Tx.fromRaw(validatorLogout.hex()), validatorLogout);
  });

  it('should allow to create and parse validatorLeave tx.', () => {
    const slotId = 0;
    const tenderKey = '0x7640D69D9EDB21592CBDF4CC49956EA53E59656FC2D8BBD1AE3F427BF67D47FA';
    const validatorLeave = Tx.validatorLeave(slotId, tenderKey, 10);

    assert.deepEqual(Tx.fromRaw(validatorLeave.hex()), validatorLeave);
  });
  ...
  it('should allow to create and parse periodVote tx.', () => {
    ...
    const vote = Tx.periodVote(merkleRoot, 12);
    vote.sign(PRIV);

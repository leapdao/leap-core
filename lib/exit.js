...
export default class Exit {
  ...
  static parseTxDataFromProof(proof) {
    let buf = Buffer.alloc(proof.length * 32);
    let offset = 0;
    for (let i = 0; i < proof.length; i++) {
      Buffer.from(proof[i].replace('0x', ''), 'hex').copy(buf, offset, 0, 32);
      offset += 32;
    }
    const buf2 = Buffer.from(proof[1].replace('0x', ''), 'hex');
    offset = buf2.readUInt8(0);
    const length = buf2.readUInt16BE(2);

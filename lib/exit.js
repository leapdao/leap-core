...
export default class Exit {
  ...
  static parseTxDataFromProof(proof) {
    let buf = Buffer.from(proof[1].replace('0x', ''), 'hex');
    let offset = buf.readUInt8(0);
    const sliceCount = buf.readUInt8(1);
    const length2 = buf.readUInt16BE(2);

    const txData = Buffer.alloc(length2);
    offset -= 32;
    buf.copy(txData, 0, offset, buf.length);
    offset = 32 - offset;
    for (let i = 2; i <= sliceCount + 1; i++) {
      buf = Buffer.from(proof[i].replace('0x', ''), 'hex');
      buf.copy(txData, offset, 0, 32);
      offset += 32;
    }
    return txData;
  }

...
export default class Exit {
  ...
  static async fastSellUTXO(utxo, plasmaChain, rootChain, marketMakerUrl, signer) {
  ...
  static async fastSellAmount(account, amount, color, plasmaChain, rootChain, marketMakerUrl, signer) {
  ...
  static async fastSell(spendTx, plasmaChain, rootChain, marketMakerUrl, signer) {
    const amount = bi(spendTx.outputs[0].value);
    
    const signedTx = signer
      ? signer.signTx(spendTx)
      : await spendTx.signWeb3(rootChain);
    ...
    const sig = signer 
      ...
      : await Tx.signMessageWithWeb3(rootChain, sigHash);

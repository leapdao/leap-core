...
export default class Transaction {
  ...
  static validatorLeave(slotId, tenderKey, eventsCount) {
    return new Transaction(
  ...
  static periodVote(merkleRoot, slotId, v, r, s) {
    const options = {
      merkleRoot,
      slotId,
    };
    ...
    return new Transaction(Type.PERIOD_VOTE, [], [], options);
  }
  ...
  * */
  toRaw() {
    ...
    if (this.type === Type.DEPOSIT) {
      ...
      payload.writeUInt8(this.options.slotId, 34);
      payload.writeUInt8(this.options.v, 35);
      payload.write(this.options.r.replace('0x', ''), 36, 'hex');
      payload.write(this.options.s.replace('0x', ''), 68, 'hex');
  ...
  static fromRaw(transaction) {
    ...
    switch (type) {
      ...
      case Type.PERIOD_VOTE: {
        if (dataBuf.length !== 100) {
          throw new Error('malformed periodVote tx.');
        }
        const merkleRoot = `0x${dataBuf.slice(2, 34).toString('hex')}`;
        const slotId = dataBuf.readUInt8(34);

...
export default class Transaction {
  ...
  */
  ...
  static sigHashBufStatic(type, raw, inputsLength) {
    ...
    return ethUtil.keccak256(noSigs);
  }
  ...
  sigHashBuf() {
    if (this.type === Type.SPEND_COND) {
      ...
      for (let i = 0; i < this.inputs.length; i += 1) {
        if (this.inputs[i].msgData) {
          bytesStripped += this.inputs[i].msgData.length;
        }
      }
      const raw = this.toRaw();
      const noSigs = Buffer.alloc(raw.length - bytesStripped, 0);
      let readOffset = 0;
      let writeOffset = 0;
      // copy type and lengths
      ...
      for (let i = 0; i < this.inputs.length; i += 1) {
        raw.copy(noSigs, writeOffset, readOffset, readOffset + 37);
        const msgLength = raw.readUInt16BE(readOffset + 37);
        readOffset += 37 + 2 + msgLength;
        writeOffset += 37 + 2;
      ...
      return ethUtil.ripemd160(noSigs);
    }
  ...
  sign(privKeys) {
    ...
    for (let i = startIdx; i < privKeys.length; i++) {
      const sigHashBuf = ethUtil.hashPersonalMessage(this.sigHashBuf());
      const sig = ethUtil.ecsign(
        sigHashBuf,
  ...
  static signHashWeb3(web3, hash, accountIndex = 0) {
    ...
    return new Promise((resolve, reject) => {
      web3.eth.getAccounts((err, accounts) => {
        ...
        const args = version.startsWith('0') ? [accounts[accountIndex], hash] : [hash, accounts[accountIndex]];
  ...
  signWeb3(web3, accountIndex = 0) {
    ...
    if (this.type === Type.PERIOD_VOTE) {
      return Transaction.signHashWeb3(web3, this.inputs[0].prevout.hash, accountIndex)
        .then(({ r, s, v, signer }) => {
    ...
    return Transaction.signHashWeb3(web3, this.sigHash(), accountIndex)
      .then(({ r, s, v, signer }) => {
        const inputs = this.type === Type.TRANSFER ? this.inputs : this.inputs.slice(1);
        inputs.forEach((input) => {
  ...
  * */
  ...
  static fromRaw(transaction) {
    ...
    switch (type) {
      case Type.DEPOSIT: {
        if (dataBuf.length !== (OUT_LENGTH + 6)) {
      ...
      case Type.TRANSFER: {
        ...
        const sigHashBuf = ethUtil.hashPersonalMessage(
          Transaction.sigHashBufStatic(type, dataBuf, insLength),
        );
      ...
      case Type.SPEND_COND: {
        ...
        let offset = 2;
        for (let i = 0; i < insLength; i += 1) {
          const input = Input.fromRaw(dataBuf, offset, Type.SPEND_COND);
          ins.push(input);
          offset += 33 + 4 + 2 + 2 + input.msgData.length + input.script.length;
        }
        const outs = [];
        for (let i = 0; i < outsLength; i += 1) {
          outs.push(Output.fromRaw(

...
export default class Transaction {
  ...
  static periodVote(slotId, input) {
    input.setType(Type.PERIOD_VOTE);
  ...
  */
  ...
  static sigDataBufStatic(type, raw, inputsLength) {
    ...
    for (let i = 0; i < inputsLength; i += 1) {
      ...
      if (type === Type.TRANSFER) {
        ...
        const msgDataLength = raw.readUInt16BE(offset + 98);
        const scriptLength = raw.readUInt16BE(offset + 100 + msgDataLength);
        offset += 98;
        raw.copy(noSigs, offset, offset, offset + 2 + 2 + msgDataLength + scriptLength);
        offset += 2 + 2 + msgDataLength + scriptLength;
      } else {
        offset += 33;
      }
    }
  ...
  sigHashBuf() {
    const hash = ethUtil.keccak256(this.sigDataBuf());
    if (this.type === Type.SPEND_COND) {
      return ethUtil.ripemd160(hash);
    }
    return hash;
  }
  ...
  static fromRaw(transaction) {
    ...
    switch (type) {
      ...
      case Type.SPEND_COND: {
        ...
        let offset = 2;
        for (let i = 0; i < insLength; i += 1) {
          const input = Input.fromRaw(dataBuf, offset, sigHashBuf, type);
          ins.push(input);
          offset += 33 + 65 + 2 + 2 + input.msgData.length + input.script.length;
        }
        const outs = [];
        for (let i = 0; i < outsLength; i += 1) {
          outs.push(Output.fromRaw(

  ...
  * */
  toRaw() {
    ...
    if (this.type === Type.DEPOSIT) {
      ...
      payload.writeUInt16BE(this.options.slotId, 2);
      payload.write(this.options.tenderKey.replace('0x', ''), 4, 'hex');
      payload.writeUInt32BE(this.options.activationEpoch, 36);
  ...
  static fromRaw(transaction) {
    ...
    switch (type) {
      ...
      case Type.VALIDATOR_LEAVE: {
        ...
        return Transaction[txFunction](slotId, tenderKey);
      }
      case Type.VALIDATOR_LOGOUT: {
        ...
        const activationEpoch = dataBuf.readUInt32BE(36);
        return Transaction.validatorLogout(slotId, tenderKey, activationEpoch);
      }
      case Type.EXIT: {

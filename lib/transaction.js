...
export default class Transaction {
  ...
  */
  ...
  static signHashWeb3(web3, hash) {
    return web3.eth.getAccounts().then(([address]) => {
      if (!address) {
        throw new Error('No unlocked accounts');
      }

      return web3.eth.sign(hash, address).then((sig) => {
        const { r, s, v } = ethUtil.fromRpcSig(sig);

        return { r, s, v, signer: address };
      });
    });
  }
  ...
  signWeb3(web3) {
    ...
    if (this.type === Type.PERIOD_VOTE) {
      return Transaction.signHashWeb3(web3, this.inputs[0].prevout.hash)
        .then(({ r, s, v, signer }) => {
    ...
    return Transaction.signHashWeb3(web3, this.sigHash()).then(({ r, s, v, signer }) => {

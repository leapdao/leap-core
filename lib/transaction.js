...
export default class Transaction {
  ...
  */
  ...
  static signHashWeb3(web3, hash, accountIndex = 0) {
    return web3.eth.getAccounts().then((accounts) => {
      if (!accounts[accountIndex]) {
        throw new Error(`No account at index ${accountIndex}`);
      }

      return web3.eth.sign(hash, accounts[accountIndex]).then((sig) => {
        const { r, s, v } = ethUtil.fromRpcSig(sig);

        return { r, s, v, signer: accounts[accountIndex] };
      });
    });
  }
  ...
  signWeb3(web3, accountIndex = 0) {
    ...
    if (this.type === Type.PERIOD_VOTE) {
      return Transaction.signHashWeb3(web3, accountIndex, this.inputs[0].prevout.hash)
        .then(({ r, s, v, signer }) => {
    ...
    return Transaction.signHashWeb3(web3, accountIndex, this.sigHash())
      .then(({ r, s, v, signer }) => {

...
export default class Transaction {
  ...
  static validatorJoin(slotId, signerAddr, tenderKey, eventsCount) {
  ...
  static validatorLeave(slotId, signerAddr, tenderKey, eventsCount) {
    return new Transaction(
  ...
  * */
  toRaw() {
    ...
    } else if (this.type === Type.VALIDATOR_JOIN || this.type === Type.VALIDATOR_LEAVE) {
      payload = Buffer.alloc(60, 0);
      payload.writeUInt8(this.type, 0);
      payload.writeUInt8(0, 1); // 0 inputs, 0 output
      payload.writeUInt16BE(this.options.slotId, 2);
      payload.write(this.options.signerAddr.replace('0x', ''), 4, 'hex');
      payload.write(this.options.tenderKey.replace('0x', ''), 24, 'hex');
      payload.writeUInt32BE(this.options.eventsCount, 56);
  ...
  static fromRaw(transaction) {
    ...
    switch (type) {
      ...
      case Type.VALIDATOR_LEAVE: {
        ...
        const signerAddr = `0x${dataBuf.slice(4, 24).toString('hex')}`;
        const tenderKey = `0x${dataBuf.slice(24, 56).toString('hex')}`;
        const eventsCount = dataBuf.readUInt32BE(56);
        return Transaction[txFunction](slotId, signerAddr, tenderKey, eventsCount);
      }

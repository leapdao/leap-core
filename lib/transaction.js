...
export default class Transaction {
  ...
  static validatorJoin(slotId, tenderKey) {
    return new Transaction(
      Type.VALIDATOR_JOIN, [], [], { slotId, tenderKey: tenderKey.toLowerCase() },
    );
  }

  static validatorLogout(slotId, tenderKey, activationEpoch) {
    return new Transaction(
      Type.VALIDATOR_LOGOUT, [], [], { slotId, tenderKey, activationEpoch },
    );
  }

  static validatorLeave(slotId, tenderKey) {
    return new Transaction(
      Type.VALIDATOR_LEAVE, [], [], { slotId, tenderKey },
    );
  }
  ...
  * */
  toRaw() {
    ...
    if (this.type === Type.DEPOSIT) {
      ...
      payload.writeUInt16BE(this.options.slotId, 2);
      payload.write(this.options.tenderKey.replace('0x', ''), 4, 'hex');
      payload.writeUInt32BE(this.options.activationEpoch, 2);
  ...
  static fromRaw(transaction) {
    ...
    switch (type) {
      ...
      case Type.VALIDATOR_JOIN: {
        ...
        const tenderKey = `0x${dataBuf.slice(4).toString('hex')}`;
        return Transaction.validatorJoin(slotId, tenderKey);
      }
      case Type.EXIT: {
      ...
      case Type.COMP_REQ: {
        ...
        offset += 16 + outs[0].msgData.length;

  ...
  */
  ...
  sigHashBuf() {
    if (this.type === Type.SPEND_COND) {
      ...
      console.log('bytesStripped: ', bytesStripped);
      ...
      for (let i = 0; i < this.inputs.length; i += 1) {
        console.log('noSigs: ', noSigs.toString('hex'));
        raw.copy(noSigs, writeOffset, readOffset, readOffset + 33);
        const msgLength = raw.readUInt16BE(readOffset + 33);
        readOffset += 33 + 2 + msgLength;
        writeOffset += 33 + 2;
        console.log('i: ', i, 'readOffset: ', readOffset, 'writeOffset: ', writeOffset, 'msgLength: ', msgLength);
    ...
    switch (type) {
      ...
      case Type.SPEND_COND: {
        ...
        for (let i = 0; i < insLength; i += 1) {
          ...
          ins.push(input);
          offset += 33 + 2 + 2 + input.msgData.length + input.script.length;
        }
        const outs = [];
        for (let i = 0; i < outsLength; i += 1) {
          const out = Output.fromRaw(dataBuf, offset);

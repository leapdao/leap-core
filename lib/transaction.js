...
export default class Transaction {
  ...
  /**
  * Takes value, addr and encodes them the following way (29 bytes total):
  *
  * 0 -   1 byte type
  * 1 -   8 bytes value
  * 9 -  20 bytes address
  ...
  * */
  ...
  /**
  * Takes depositId, value, addr and encodes them the following way (33 bytes total):
  *
  * 0 -   1 byte type
  * 1 -   4 bytes depositId
  * 5 -   8 bytes value
  ...
  * */
  ...
  /**
  * Takes inputs and outputs and encodes them the following way (262 bytes total):
  *
  * 0 -   1 byte type
  * 1 -   8 bytes height
  ...
  * 43 -  32 bytes r-signature        #
  * 75 -  32 bytes s-signature        #    second signature
  * 107 - 1 byte v-signature          #
  * 206 - 8 bytes value             $
  ...
  static parse(transaction) {
    ...
    switch (type) {
      ...
      case Type.TRANSFER: {
        ...
        const transfer = new Transaction(height).transfer(ins, outs);

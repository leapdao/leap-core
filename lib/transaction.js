...
export default class Transaction {
  ...
  static consolidate(inputs, output) {
    if (!(output instanceof Output)) {
      throw Error('second argument should be of type Output');
    }
  ...
  */
  ...
  sign(privKeys) {
    ...
    const startIdx = (this.type === Type.TRANSFER || this.type === Type.CONSOLIDATE) ? 0 : 1;
  ...
  signWeb3(web3, accountIndex = 0) {
    ...
    return Transaction.signMessageWithWeb3(web3, this.sigData(), accountIndex)
      .then(({ r, s, v, signer }) => {
        const inputs = (this.type === Type.TRANSFER || this.type === Type.CONSOLIDATE) 
          ? this.inputs
          : this.inputs.slice(1);
        inputs.forEach((input) => {
  ...
  static fromRaw(transaction) {
    ...
    switch (type) {
      ...
      case Type.PERIOD_VOTE: {
        ...
        ins.push(Input.fromRaw(dataBuf, 3, sigHashBuf, type));
      ...
      case Type.TRANSFER: {
        ...
        for (let i = 0; i < insLength; i += 1) {
          ins.push(Input.fromRaw(dataBuf, 2 + (i * SPEND_INPUT_LENGTH), sigHashBuf, type));
        }
        const outs = [];
      ...
      case Type.SPEND_COND: {
        ...
        let offset = 2;
        for (let i = 0; i < insLength; i += 1) {
          const input = Input.fromRaw(dataBuf, offset, undefined, Type.SPEND_COND);
          ins.push(input);

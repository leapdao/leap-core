...
describe('checkSpendCond', () => {
  ...
  it('valid tx', async () => {
    ...
    const outputs = await simSpendCond(condition, testErc20s, state)
  ...
  it('valid tx with 2 colors, 4 script inputs, color 1 with aggregation outputs', async () => {
    ...
    const outputs = JSON.stringify(await simSpendCond(condition, testErc20s, state))
    ...
    const expected = `{"outputs":[
      ...
      {"address":"0x${ethUtil.ripemd160(conditionScript4).toString('hex')}","value":"596000","color":"2"}],

...
describe('Input', () => {
  ...
  it('should allow to parse spending condition input', () => {
    const inputBuf = Buffer.from('0d000000000754d4ec117777777777777777777777777777777777777777777777777777777777777777000001aa0002bbbb', 'hex');
    ...
    assert.equal(input.toRaw().toString('hex'), '7777777777777777777777777777777777777777777777777777777777777777000001aa0002bbbb');
  });

  it('should allow to parse spending condition input with no data', () => {
    const inputBuf = Buffer.from('0d000000000754d4ec1177777777777777777777777777777777777777777777777777777777777777770100000000', 'hex');
    const input = Input.fromRaw(inputBuf, 10, Type.SPEND_COND);
    assert.deepEqual(input.toJSON(), {
      hash: `0x${inputBuf.slice(10, 42).toString('hex')}`,
      index: 1,
    ...
    assert.equal(input.toRaw().toString('hex'), '77777777777777777777777777777777777777777777777777777777777777770100000000');
  });
});

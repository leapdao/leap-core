...
export default class Input {
  ...
  toRaw() {
    ...
    if (this.isDeposit()) {
      ...
      let offset = 33;
      const msgDataLen = this.msgData ? this.msgData.length : 0;
      offset = dataBuf.writeUInt16BE(msgDataLen, offset);
      if (msgDataLen) {
        offset += this.msgData.copy(dataBuf, offset);
      }

      offset = dataBuf.writeUInt16BE(this.script.length, offset);
      this.script.copy(dataBuf, offset);
  ...
  static fromRaw(buf, offset, txType) {
    ...
    if (txType === Type.SPEND_COND) {
      ...
      return new Input({ prevout, msgData, script, type: txType });
    }
    ...
    if (txType) {
      ...
      const signer = Input.recoverSignerAddress(txType, v, r, s);

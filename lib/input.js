...
export default class Input {
  constructor(options) {
    ...
    } else if (options && options.msgData) {
  ...
  isConsolidation() {
    return (this.type === Type.CONSOLIDATE);
  }
  ...
  getSize() {
    ...
    if (this.type === Type.SPEND_COND) {
      ...
      return 33 + 4 + 4 + this.msgData.length + this.script.length;
    }
    if (this.isSpend()) {
  ...
  toJSON() {
    ...
    if (this.isConsolidation()) {
      const input = { ...this.prevout, type: this.type };
      input.hash = ethUtils.bufferToHex(input.hash);
      return input;
    }
    if (this.isSpend()) {
  ...
  toRaw() {
    ...
    if (this.isDeposit()) {
      ...
      dataBuf.writeUInt16BE(this.script.length, 39 + this.msgData.length);
      this.script.copy(dataBuf, 41 + this.msgData.length);
  ...
  static fromRaw(buf, offset, sigHashBuf) {
    ...
    let input;
    if (sigHashBuf === Type.CONSOLIDATE
      || sigHashBuf === Type.COMP_RESP || sigHashBuf === Type.COMP_REQ) {
      // nothing
      input = new Input({ prevout, type: sigHashBuf });
    } else if (sigHashBuf === Type.SPEND_COND) {
      const gasPrice = buf.readUInt32BE(off + 33);
      const msgLength = buf.readUInt16BE(off + 37);
      const msgData = Buffer.alloc(msgLength);
      buf.copy(msgData, 0, off + 39, off + 39 + msgLength);
      
      const scriptLength = buf.readUInt16BE(off + 39 + msgLength);
      const script = Buffer.alloc(scriptLength);
      buf.copy(script, 0, off + 41 + msgLength, off + 41 + msgLength + scriptLength);
      input = new Input({ prevout, gasPrice, msgData, script, type: sigHashBuf });
    } else if (sigHashBuf) {

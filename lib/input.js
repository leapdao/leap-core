...
export default class Input {
  ...
  getSize() {
    ...
    if (this.isSpend() || this.isConsolidation()) {
  ...
  static fromRaw(buf, offset, sigHashBuf, type) {
    ...
    if (type === Type.SPEND_COND) {
      ...
      input = new Input({ prevout, gasPrice, msgData, script, type });
    } else if (sigHashBuf) {
      if (type === Type.CONSOLIDATE) {
        input = new Input({ prevout, type });
      } else {

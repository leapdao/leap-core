...
export default class Input {
  ...
  getSize() {
    ...
    if (this.type === Type.SPEND_COND) {
      // prevout + sig + 2 bytes msgData length + msgData
      // + 2 bytes script length + script
      let rv = 33 + 65 + 2 + 2 + this.script.length;
  ...
  setScript(script) {
    assert(this.type === Type.SPEND_COND, 'Can only set script on SPEND_COND');
    this.script = script;
    if (!Buffer.isBuffer(script)) {
      this.script = ethUtils.toBuffer(script);
    }
  }
  ...
  toRaw() {
    ...
    if (this.isDeposit()) {
      ...
      if (msgDataLength) {
        dataBuf.writeUInt16BE(this.msgData.length, 98);
        this.msgData.copy(dataBuf, 100);
      }
      dataBuf.writeUInt16BE(this.script.length, 100 + msgDataLength);
      this.script.copy(dataBuf, 102 + msgDataLength);
   ...
   * @returns {Input}
   */
  static fromRaw(buf, offset, sigHashBuf, isSpendCond) {
    const off = (offset) || 0;
    ...
    if (sigHashBuf) {
      ...
      r = buf.slice(33 + off, 65 + off);
      s = buf.slice(65 + off, 97 + off);
      v = buf.readUInt8(97 + off);
      signer = Input.recoverSignerAddress(sigHashBuf, v, r, s);
      input.setSig(r, s, v, signer);
      if (isSpendCond) {
        const msgLength = buf.readUInt16BE(98 + off);
        const msgData = Buffer.alloc(msgLength);
        buf.copy(msgData, 0, off + 100, off + 100 + msgLength);

        const scriptLength = buf.readUInt16BE(off + 100 + msgLength);
        const script = Buffer.alloc(scriptLength);
        buf.copy(script, 0, off + 102 + msgLength, off + 102 + msgLength + scriptLength);
        input = new Input({
          prevout,
          script,
          msgData,

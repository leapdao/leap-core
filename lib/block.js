...
export default class Block {
  ...
  static from(height, timestamp, txList) {
    return new Block(height, {
      timestamp,
      txs: txList.map(tx => Tx.fromRaw(tx.raw)),
    });
  }
  ...
  header(payload) {
    ...
    payload.write(
      this.merkleRoot()
        .replace('0x', '')
        .padStart(2, '0'), // fix for Invalid hex string error in browsers if merkleRoot is "0"
      8,
      'hex',
    );
    return payload;
  ...
  hash() {
    ...
    return ethUtil.bufferToHex(ethUtil.keccak256(payload));
  }
  ...
  equals(another) {
    this.getMerkleTree().getRoot().equals(another.getMerkleTree().getRoot());
  }
  ...
  */
  ...
  proof(tx, proofOffset = 0) {
    ...
    if (proofOffset > 0) {
      ...
      let nextPow = (this.txList.length % 2) ? this.txList.length : this.txList.length - 1;
    ...
    const metadataAndTxLength = 12 + txData.length;

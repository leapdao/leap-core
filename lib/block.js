...
export default class Block {
  ...
  addTx(tx) {
    if (this.txList.findIndex(compHash(tx.hash())) > -1) {
  ...
  proof(txData, pos, proof) {
    ...
    // offset
    let offset = ((txData.length % 32) <= 19) ? 32 - (txData.length % 32) : (32 - (txData.length % 32)) + 32;
    offset += 96;
    payload.writeUInt8(offset, 0);
    // proof offset in array

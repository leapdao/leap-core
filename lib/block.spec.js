...
describe('blocks', () => {
  ...
  it('should allow to create block with deposit.', (done) => {
    ...
    const block = new Block(height);
    block.addTx(deposit);
    expect(block.merkleRoot()).to.eql('0x16ded81eb8c50a1433b729538d44f63314dfe5451019467035bbd9748f9b7f2c');
    // test hashing
    expect(block.hash()).to.eql('0x9aa9bb11540da455ef4157b876f9c9ee4128076d1fd950beca8fb4beb599c12e');

    // create proof

    const proof = block.proof(deposit);
    expect(proof).to.eql([
      '0x9aa9bb11540da455ef4157b876f9c9ee4128076d1fd950beca8fb4beb599c12e',

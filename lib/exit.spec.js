...
describe('exit', () => {
  ...
  const depositTx = Tx.deposit(12, 12, ADDR);
  const utxoId = (new Outpoint(depositTx.hash(), 0)).getUtxoId();
  const value = 1000;
  ...
  it('can parse tx data from proof', () => {
    ...
    assert.deepEqual(txData, transfer.toRaw());
  });
  ...
  it('can construct tx from proof', () => {
    ...
    // TODO: write equality checker for transactions to assert
